"""Tests for `store_traces.py` (the write/read round trip). Analytical-query
tests (`nearest_successful`, `prune_attributes`) live in
`test_store_traces_query.py`, mirroring the `store_traces.py` /
`store_traces_query.py` module split.
...
def test_step_from_row_defaults_null_actor_and_summary_to_empty_string():
    ...
    must not crash if one exists (e.g. hand-inserted test data). See
    `_step_from_row`'s docstring for which side of this mismatch is
    recommended to change in revision 0002."""
    row = {
        "trace_id": "t1",
        "step_index": 0,
        "span_id": "t1-sp0",
...
# --- offline: write_trace / read_trace SQL shape --------------------------
